...
This module provides functions for calculating orbital parameters and properties, including conversions between orbital period and semi-major axis using Kepler's laws, as well as post-supernova orbital dynamics for binary systems. The post-supernova calculations include changes in orbital separation, eccentricity, and systemic velocity due to kicks imparted during supernova events.

Key Features:
- Keplerian orbital parameter calculations.
- Post-supernova orbital properties for circular and general pre-explosion orbits.
- Radial velocity semi-amplitude calculations.

References:
- Tauris et al. (1999): Monthly Notices of the Royal Astronomical Society, Volume 310, Issue 4, pp. 1165-1169.
- Willcox, Marchant, Vigna-Gomez, et al. (2025).
"""
...
"""
    post_supernova_circular_orbit_Δv(;m1_i, m2_i, a_i, m1_f=-1, m2_f, vkick=0, θ=0, ϕ=0, vimp=0)
...
# Output:
- `Δv`: post-explosion systemic velocity              [cm/s]
"""
function post_supernova_circular_orbit_Δv(;m1_i, m2_i, a_i, m1_f=-1, m2_f, vkick=0, θ=0, ϕ=0, vimp=0)
    if m1_f == -1
        m1_f = m1_i
    end
    v_rel = relative_velocity(m1=m1_i, m2=m2_i, a=a_i)
    ...
    Δv = sqrt(Δp_x^2 + Δp_y^2 + Δp_z^2)/(m2_f + m1_f)
...
using equations from Willcox, Marchant, Vigna-Gomez, et al. 2025 
...
function post_supernova_general_orbit_parameters(;m1_i, m2_i, a_i, e_i=0, m1_f=-1, m2_f, vkick=0, 
    ...
    v_par = 1/M_f* (v_reli/M_i* (m1_i*m2_f - m1_f*m2_i)
                      + m2_f*vkick*cosθ + h_ν*m1_f*vimp)
    v_per = 1/M_f* (m2_f*vkick*sinθ*cosϕ + j_ν*m1_f*vimp)
    v_z   = 1/M_f* (m2_f*vkick*sinθ*sinϕ)
    ...
    # Compute longitude of the ascending node. This is obtained by taking the vector Ωvec_f = Lvec_f × \hat{O} 
    # which is L_n*\hat{W} - L_w*\hat(N). Then Ωvec_f = (Ω_w, Ω_n, 0) points to the ascending node, and we have,
    Ω_w = L_n 
    Ω_n = -L_w
    ...
    vcm_1f_dot_y = vimp - h_ν*(m2_i/M_i*v_reli + v_par) - j_ν*v_per
    if vcm_1f_dot_y<0
        ν_f = 2π - ν_f
    end
    ...
    # We need to know if ν is before or after the ascending node. To do this we take 
    # the cross product of the vector pointing from the COM to star 1, and that 
    # pointing to the ascending node. If this points in the direction of the orbital 
    # angular momentum, then we have not overtaken the ascending node.
    ...
    return (a_f, e_f, Ω_f, ω_f, i_f, Δv_α, Δv_δ, Δv_r)
end
